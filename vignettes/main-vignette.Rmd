...
The **countries** vector indicates the country of each individual's sampling site.
...
The **plot** function with argument method = "map.max" or method = "map.all" interpolates the values of the $Q$-matrix on a geographic map. The map can be infered from the sample coordinates, or it can be provided to the program in a raster format. 
...
Legend can be added to the map using **legend =TRUE**, and personalized via optional tuning parameters.

```{r map.plot.leg, dependson=c("run")} 
plot(q.matrix, coordinates, method = "map.max", cex = .4,  
     interpol = kriging(10), raster.filename = asc.raster,
     main = "Ancestry coefficients", xlab = "Longitude", ylab = "Latitude", 
     col.palette = my.palette, legend = TRUE,
     horizontal = FALSE, legend.width = 2.8, legend.ncol = 1, legend.space = c(8,3),
     leg.extra.args = list(legend.lab = "Anc. coef.", legend.cex = .5, legend.line = 1.5, 
                           axis.args = list(cex.axis = .5)))
```

The **plot** function with argument **method = "piechart"** or **method = "piechart.pop"** displays piecharts on a map at each sampling location. It can be displayed on top of a previous plot using **add = TRUE**, or on its own using **add = FALSE**.

```{r map.piechart, dependson=c("run")} 
plot(q.matrix, coordinates, method = "map.max", cex = .4,  
     interpol = kriging(10), 
     raster.filename = asc.raster,
     main = "Ancestry coefficients",
     xlab = "Longitude", ylab = "Latitude", 
     col.palette = my.palette)
plot(q.matrix, coordinates, method = "piechart", col.palette = my.palette, add = TRUE)
```

When **method = "piechart.pop"** the piecharts are displayed for groups rather than individuals. Each piechart represents the average ancestry coefficients of all individuals belonging to the same group. In the following example the group labels are given as the countries where each plant was sampled. Moreover, the piechart size can be scaled to the sample size of each group using **scale = TRUE** and argument **radius**.

```{r map.piechart.pop, dependson=c("run")} 
# (not mandatory) Select only 2 first letters of country names 
# used for printing, not for grouping:
country.codes <- substr(unique(countries), 1, 2) 
plot(q.matrix, coordinates, method = "piechart.pop",
     main = "Ancestry coefficients",
     xlab = "Longitude", ylab = "Latitude", 
     pop = countries, names.pie = country.codes, 
     scale = TRUE, radius = .04, cex = .9, col.palette = my.palette,
     leg.bubble.args = list(x = "topleft", txt.cex = .6))
text(-11,59,"Sampling\nsize", cex = .6)
